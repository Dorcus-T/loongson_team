...
module if_stage (
    ...
    output wire                     if_ready_go,         // IF阶段就绪标志
    // 与 ICache 的接口
    ...
    wire        pre_if_allowin;              // pre_if阶段允许接收新指令
    // ========== 控制信号解析 ==========
    ...
    // ========== 流水线控制 ==========
    assign pre_if_allowin     = (if_allowin || !pre_if_valid) && pre_if_ready_go || flush_active;  // 预取指阶段允许接收新指令的条件：1. pre_if阶段无效；2. pre_if阶段有效且就绪
    assign pre_if_valid       = ~reset;                                             // 预取指阶段：只要不复位就一直有效
    assign pre_if_ready_go    = (icache_cpu_req && icache_cpu_addr_ok) || req_already || pre_if_exc_valid || flush_active || !pre_if_valid;
    // 如果下一次上跳能够握手发请求或者已经发送过请求，那么下一次上跳就可以前进
    assign seq_pc  = pre_if_pc_r + 32'h4;                 // 顺序PC = pre_if PC + 4（指令长度4字节）
    assign nextpc  = exc_no_rf     ? exc_entry      :                             // WB阶段有异常就进入异常处理地址，WB为ertn则返回原来地址，此两种之后再考虑跳转
                     ...
                     ex_mispredict ? ex_corr_target :                          // EX 级统一误预测纠正
                                     seq_pc      ;
    // nextpc逻辑中异常和ertn的优先级高于brtaken，如果ex和wb同时发来信号，优先处理wb的信号
    wire flush_active; 
    assign flush_active = ex_mispredict || exc_no_rf || wb_ertn_flush || rf_valid;
    assign if_ready_go    = (flush_active || !if_valid || (icache_cpu_data_ok && if_valid) || if_exc_valid) && !inst_dirty;
    // 分支/跳转会阻塞if指令前进；数据从桥直连进入if，不再经if_inst_r暂存
    assign if_allowin     = if_ready_go && (id_allowin || !if_valid) && pre_if_ready_go;
    // 分支让if不走但能进，让if被替换；冲刷则是让正确指令能进就行
    assign if_to_id_valid = if_valid;

    // 取值阶段有效标志更新
    always @(posedge clk) begin
        if (reset || flush_active) begin
            if_valid <= 1'b0;
        end
    ...
    // ========== ICache 请求控制 ==========
    always @(posedge clk) begin
        if (reset || pre_if_allowin || flush_active) begin
            req_already <= 1'b0;
        end
        else if ((icache_cpu_req && icache_cpu_addr_ok) && !pre_if_allowin) begin
            req_already <= 1'b1;
        end
    end
    ...
    assign inst_dirty_control = ((!if_exc_valid && if_valid && !icache_cpu_data_ok) && req_already) ? 3'b010 :
                                ((!if_exc_valid && if_valid && !icache_cpu_data_ok) || req_already) ? 3'b001 :
                                                                                                      3'b000 ;
    always @(posedge clk) begin
        if (reset) begin
            inst_dirty <= 3'b0;
    ...
    // ========== ICache 输出信号 ==========
    assign if_to_mmu_vaddr = pre_if_pc_r;                              // 发mmu虚地址
    ...
    assign icache_cpu_index  = pre_if_pc_r[`OFFSET_WIDTH +: `INDEX_WIDTH]; // 虚地址中的index部分
    assign icache_cpu_tag    = padd[`OFFSET_WIDTH + `INDEX_WIDTH +: `TAG_WIDTH];             // 实地址中的tag部分
    assign icache_cpu_offset = pre_if_pc_r[0 +: `OFFSET_WIDTH];           // 虚地址中的offset部分
    ...
    assign icache_cpu_accept = if_allowin;                                                // IF阶段能接受数据的条件：1. IF到ID的总线有效且ID允许接收；2. 当前指令是脏指令（需要被冲刷掉）
    ...
    assign if_exc_valid = |if_exc && if_valid;
endmodule
