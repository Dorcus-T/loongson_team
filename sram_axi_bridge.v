...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (ar_state == AR_IDLE) begin
            ...
            end else if (if_rd_req && inst_sram_addr_ok) begin
                arvalid_r <= 1'b1;
                arid_r    <= 4'd0;
                araddr_r  <= inst_sram_addr;
                arsize_r  <= map_size(inst_sram_size);
            end
        end else if (ar_state == AR_REQ && arvalid_r && arready) begin
            arvalid_r <= 1'b0;
        end
    end
end
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (aw_w_state == AW_W_IDLE && mem_wr_req && data_sram_addr_ok_wr) begin
            ...
            aw_done_r <= 1'b0;
            w_done_r  <= 1'b0;
        end else if (aw_w_state == AW_W_BUSY) begin
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (aw_w_done) begin
            ...
            end else if (!wr_pend1_valid) begin
                wr_pend1_valid <= 1'b1;
